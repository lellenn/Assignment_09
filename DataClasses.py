...
# LWarner, 2021-Dec-11, Modified code and added code for the Track Class 
#------------------------------------------#
...
class Track():
    ...
    def position(self, position):
        if str(position).isnumeric():
            if position < 1:
                raise Exception('Position must be greater than zero!')
            return position
        else:
...
class CD:
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        """
        try:
            self.__cd_tracks.remove(track_id)
        except ValueError:
            print("Removal did not succeed")
        self.__sort_tracks()
