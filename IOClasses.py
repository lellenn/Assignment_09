...
#LWarner, 2021-Dec-11, Modified code per TODO tasks
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            ...
            TrkFile = open(file_name_track, 'w')
            for disc in lst_Inventory:
    ...
    def load_inventory(file_name: list) -> list:
        ...
        file_name_track = file_name[1]
        try:
            CDFile = open(file_name_CD, 'r')
            for line in CDFile:
                data = line.strip().split(',')
                objRow = DC.CD(data[0], data[1], data[2])
                lst_Inventory.append(objRow)
            CDFile.close()
            TrkFile = open(file_name_track, 'r')
            for line in TrkFile:
                data = line.strip().split(',')
